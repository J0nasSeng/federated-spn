...
def get_horizontal_train_data(ds, num_clients, partitioning="iid", dir_alpha=0.2):
    if ds in ["income", "breast-cancer", "credit"]:
        if ds == "income":
            ...
            partitioner = BreasCancerPartitioner(
                dataset.targets,
                num_clients,
                partition=partitioning,
                dir_alpha=dir_alpha,
            ...
            partitioner = GimmeCreditPartitioner(
                dataset.targets,
                num_clients,
                partition=partitioning,
                dir_alpha=dir_alpha,
...
def split_dataset_hybrid(
    data, num_clients, num_cols, min_dim_frac, max_dim_frac, sample_frac, seed
):
...
def get_hybrid_train_data(
    ds, num_clients, min_dim_frac=0.25, max_dim_frac=0.5, sample_frac=None, seed=111
):
    if ds in ["income", "breast-cancer", "credit"]:
        if ds == "income":
            dataset = Income("../../../datasets/income/", split="train")
        ...
        client_data, subspaces, client_idx = split_dataset_hybrid(
            data, num_clients, columns, min_dim_frac, max_dim_frac, sample_frac, seed
        )
        ...
        np_targets = dataset.targets.numpy()
        data = np.hstack((np_features, np_targets.reshape(-1, 1)))
        client_data, subspaces = split_dataset_hybrid(
            data, num_clients, columns, min_dim_frac, max_dim_frac, sample_frac, seed
        )
