...
def get_hyperparameters(subspaces, ds):
    ...
    for client_space in subspaces:
        cat_idxs, cat_size = [], []
        grouped_features = []
        for cat_idx, size in zip(
            hyperparam_dict["cat_idxs"], hyperparam_dict["cat_dims"]
        ):
...
def main(args):
    (
        ...
        test_targets,
        subspaces,
    ) = get_vertical_data(args.dataset, args.num_clients)
    ...
    for e in range(args.epochs):
        tt_loss = train_epoch(
            model, train_loaders, optim, criterion, scheduler, num_train_batches
        )
        avg_acc, avg_f1_micro, avg_f1_macro, val_loss = validate_epoch(
            model, test_loaders, criterion, num_test_batches
        )

        print(
            f"Epoch: {e}, Loss: {tt_loss:.4f}, Val-Loss: {val_loss:.4f}, Accuracy: {avg_acc:.4f}, F1-Micro: {avg_f1_micro:.4f}, F1-Macro: {avg_f1_macro:.4f}"
        )
