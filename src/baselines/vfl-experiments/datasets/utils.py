...
def get_vertical_data(ds, num_clients, rand_perm=False):
    ...
    if ds in ["income", "breast-cancer", "credit"]:
        if ds == "income":
            ...
            train_dataset = BreastCancer(
                "../../../datasets/breast-cancer/", split="train"
            )
            test_dataset = BreastCancer(
                "../../../datasets/breast-cancer/", split="test"
            )
        elif ds == "credit":
            train_dataset = GimmeCredit(
                "../../../datasets/GiveMeSomeCredit/", split="train"
            )
            test_dataset = GimmeCredit(
                "../../../datasets/GiveMeSomeCredit/", split="test"
            )
