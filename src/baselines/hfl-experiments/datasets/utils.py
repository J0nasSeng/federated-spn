...
class PartitionedIncome:
    def __init__(
        self, num_clients, partitioning, root="../../../datasets/income/", split="train"
    ):
        self.dataset = Income(root, split=split)
        self.partitioner = IncomePartitioner(
            self.dataset.targets, num_clients, partition=partitioning
        )
        self.in_dim = 14
        self.out_dim = 2
...
class PartitionedAvazu:
    def __init__(
        self, num_clients, partitioning, root="../../../datasets/avazu/", split="train"
    ):
        self.dataset = Avazu(root, split=split)
        self.partitioner = AvazuPartitioner(
            self.dataset.targets, num_clients, partition=partitioning
        )
        self.in_dim = 20
        self.out_dim = 2
...
class PartitionedBreastCancer:
    def __init__(
        ...
        num_clients,
        partitioning,
        root="../../../datasets/breast-cancer/",
        split="train",
    ):
        self.dataset = BreastCancer(root, split=split)
        self.partitioner = BreastCancerPartitioner(
            self.dataset.targets, num_clients, partition=partitioning
        )
        self.in_dim = 30
        self.out_dim = 2
...
class PartitionedCredit:
    def __init__(
        ...
        num_clients,
        partitioning,
        root="../../../datasets/GiveMeSomeCredit/",
        split="train",
    ):
        self.dataset = GimmeCredit(root, split=split)
        self.partitioner = CreditPartitioner(
            self.dataset.targets, num_clients, partition=partitioning
        )
        self.in_dim = 10
        self.out_dim = 2
...
class PartitionedMNIST:
    def __init__(
        self, num_clients, partitioning, root="../../../datasets/", train=True
    ):
        ...
        self.partitioner = MNISTPartitioner(
            self.dataset.targets, num_clients, partition=partitioning
        )
        self.in_dim = 28 * 28
        self.out_dim = 10
    ...
    def get_dataset(self, client_num):
        features = self.dataset.data / 255.0
        targets = self.dataset.targets
        idx = self.partitioner.client_dict[client_num]
        ds = TensorDataset(features[idx], targets[idx])
        return ds
...
class PartitionedCIFAR10:
    def __init__(
        self, num_clients, partitioning, root="../../../datasets/cifar10", train=True
    ):
        transform = torchvision.transforms.Compose(
            [
                torchvision.transforms.ToTensor(),
        ...
        self.partitioner = CIFAR10Partitioner(
            self.dataset.targets, num_clients, partition=partitioning
        )
        self.in_dim = 32 * 32
        self.out_dim = 10
    ...
    def get_dataloader(self, id, batch_size):
        features = torch.from_numpy(self.dataset.data) / 255.0
...
def get_test_dataset(ds):
    if ds == "income":
        ...
        cifar = torchvision.datasets.CIFAR10(
            root="../../../datasets/cifar10", train=False, transform=transform
        )
