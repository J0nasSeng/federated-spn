...
from client_trainers import (
    TabNetFedAvgSerialClientTrainer,
    TabNetFedProxSerialClientTrainer,
    TabNetScaffoldSerialClientTrainer,
)
    ...
    def main(self):
        ...
        while self.handler.if_stop is False:
            ...
            evaluation = self.evaluate(
                self.handler.model, self.criterion, self.test_loader
            )
            if len(evaluation) == 2:
                loss, acc = evaluation
                print(
                    "Round {}, Loss {:.4f}, Test Accuracy {:.4f}".format(t, loss, acc)
                )
            elif len(evaluation) == 4:
                loss, acc, f1_micro, f1_macro = evaluation
                print(
                    "Round {}, Loss {:.4f}, Test Accuracy {:.4f}, F1-Micro {:.4f}, F1-Macro {:.4f}".format(
                        t, loss, acc, f1_micro, f1_macro
                    )
                )
            t += 1
...
def run_pipeline(args):
    if args.partitioning == "iid":
        args.dir_alpha = 0
    dataset = get_horizontal_train_data(
        args.dataset, args.num_clients, args.partitioning
    )
    ...
    if args.dataset == "mnist":
        ...
        if args.model == "mlp":
            model = nn.Sequential(MLP(dataset.in_dim, dataset.out_dim), nn.Softmax())
        elif args.model == "tabnet":
            group_att_mat = create_group_matrix(
                tabnet_hyperparams[args.dataset]["grouped_features"], dataset.in_dim
            )
            ...
            model = TabNet(
                dataset.in_dim,
                dataset.out_dim,
                group_attention_matrix=group_att_mat,
                **tabnet_hyperparams[args.dataset],
            )
    ...
    if args.algorithm == "fedavg":
        if args.model == "tabnet":
            ...
            trainer = client.FedAvgSerialClientTrainer(
                model, args.num_clients, cuda, device
            )
    elif args.algorithm == "scaffold":
        if args.model == "tabnet":
            trainer = TabNetScaffoldSerialClientTrainer(model, args, cuda, device)
        else:
            trainer = client.ScaffoldSerialClientTrainer(
                model, args.num_clients, cuda, device
            )
        ...
        if args.model == "tabnet":
            ...
            trainer = client.FedProxSerialClientTrainer(
                model, args.num_clients, cuda, device
            )
    trainer.setup_dataset(dataset)
    trainer.setup_optim(args.epochs, args.batch_size, args.lr)

    handler = server.SyncServerHandler(
        model, args.comm_rounds, args.sample_ratio, cuda, device
    )
