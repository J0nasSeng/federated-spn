...
class ConditionalMixtureEinsum:
    def __init__(
        self, ceinsum_mixtures, device, img_dims=(112, 112, 3), patch_dims=(14, 14)
    ) -> None:
        """
...
def cluster_patches(x, y, patch_size=14, num_clusters=5):
    ...
    patch_datasets = [
        [None for _ in range(patches.shape[3])] for _ in range(patches.shape[2])
    ]
...
def train_param_nn(mixture, train_loader, num_epochs, i, j, device):
    ...
    for ep in range(1, num_epochs + 1):
        ...
        for x, y in train_loader:
            ...
            nn.utils.clip_grad.clip_grad_norm_(mixture.param_nn.parameters(), 1.0)
...
def train_conditional_mixture_einsum(dataset, epochs, clusters, cid, device_id):
    ...
    x, y = torch.cat([s for s, _ in full_loader], dim=0), torch.cat(
        [s for _, s in full_loader], dim=0
    )
    patch_datasets = cluster_patches(x, y, config.patch_size[0], num_clusters=3)
    patches = [
        [i, j]
        for i in range(len(patch_datasets))
        for j in range(len(patch_datasets[0]))
    ]
...
def sample(N, mixture_leafs, device_id):
    ...
    save_image_stack(
        samples, 5, 5, img_path, margin_gray_val=0.0, frame=2, frame_gray_val=0.0
    )


clusters = np.load(
    "/storage-01/ml-jseng/imagenet-clusters/vit_cluster_minibatch_10K.npy"
)
    ...
    if args.sample:
        ...
        for cluster_batch in unique_clusters:
            ...
            for i, rc in enumerate(cluster_batch):
                mixtures = train_conditional_mixture_einsum(
                    imagenet, 5, clusters, rc, 0
                )
                sample(25, mixtures, 0)
