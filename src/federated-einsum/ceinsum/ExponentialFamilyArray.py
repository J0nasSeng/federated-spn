...
class ExponentialFamilyArray(torch.nn.Module):
    ...
    def forward(self, x, params):
        ...
        if torch.any(torch.isnan(self.ll)):
            print(torch.any(torch.isnan(self.suff_stats)))
            print(torch.any(torch.isnan(log_normalizer)))
            print(torch.any(torch.isnan(log_h)))
            print(theta)
...
class NormalArray(ExponentialFamilyArray):
    ...
    def __init__(
        self, num_var, num_dims, array_shape, min_var=0.0001, max_var=10, use_em=True
    ):
