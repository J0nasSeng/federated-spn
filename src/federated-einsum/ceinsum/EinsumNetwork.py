from ceinsum import Graph
from ceinsum.FactorizedLeafLayer import *
from ceinsum.SumLayer import *
from utils import extract_image_patches, get_surrounding_patches
import config
...
class EinsumNetwork(torch.nn.Module):
    ...
    def forward(self, x, y, i, j):
        ...
        for sample in range(x_in.shape[0]):
            ...
            layer_params = params[0][sample]
            input_layer(x_, layer_params)
            for layer_params, einsum_layer in zip(params[1:], self.einet_layers[1:]):
                einsum_layer(layer_params[sample])
            ll = self.einet_layers[-1].prob[:, :, 0].sum()
            lls += ll
        return -(lls / x_in.shape[0])

    def backtrack(
        self, params, num_samples=1, class_idx=0, x=None, mode="sampling", **kwargs
    ):
        """
        ...
        for lidx, layer in zip(layer_idx, reversed(self.einet_layers)):
            ...
            layer_params = params[lidx][
                0
            ]  # [0] because we condition on only one sample
