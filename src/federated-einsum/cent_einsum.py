...
def train(num_epochs, device_id, chk_path):
    ...
    for epoch_count in range(num_epochs):
        ...
        einet.em_update()
    torch.save(einet, os.path.join(chk_path, f'chk_cent_einsum.pt'))
    df = pd.DataFrame(data=log_likelihoods, columns=['lls'])
    df.to_csv(os.path.join(chk_path, f'chk_{cluster_count}.csv'))
    return einet


train(config.num_epochs, config.devices[0], './checkpoints_1gpu/')