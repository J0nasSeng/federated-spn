...
from datasets.utils import (
    get_horizontal_train_data,
    get_test_data,
    get_vertical_train_data,
...
class SPFlowServer:
    def train(self, train_data, feature_spaces, args):
        ...
        for c in range(args.num_clients):
            ...
            node = FlowNode.remote(
                args.dataset,
                args.structure,
                args.num_clusters,
                args.setting,
                args.glueing,
                args.cluster_by_label,
            )
            nodes.append(node)
    ...
    def build_spn(self, feature_subspaces, nodes, args):
        ...
        if len(feature_subspaces) == 1:
            ...
            if args.glueing == "naive":
                ...
                spn, added_nodes = self.build_spn_verhyb_combinatorial(
                    feature_subspaces, nodes
                )
            return spn, added_nodes
    ...
    def build_fc(self, feature_subspaces, nodes, args):
        ...
        if len(feature_subspaces) == 1:
            ...
            if args.glueing == "naive":
                ...
                spn, added_nodes = self.build_fc_verhyb_combinatorial(
                    feature_subspaces, nodes
                )
            return spn, added_nodes
...
class EinsumServer:
    def train(self, train_data, feature_spaces, args):
        ...
        for c in range(args.num_clients):
            ...
            node = EinetNode.remote(
                args.dataset, num_classes=10, device=gpu_idx, num_epochs=args.num_epochs
            )
            nodes.append(node)
...
def main_ddt(args):
    ...
    if args.setting == "horizontal":
        train_data = get_horizontal_train_data(
            args.dataset, args.num_clients, args.sample_partitioning, args.dir_alpha
        )
        feature_spaces = [
            [list(range(train_data[0].shape[1]))] for _ in range(args.num_clients)
        ]
        feature_types = [
            [[ctxt.feature_types[args.dataset][f] for f in feature_spaces[0][0]]]
            for _ in range(args.num_clients)
        ]
        nodes = server.train(train_data, feature_spaces, feature_types, args)
    elif args.setting == "vertical":
        train_data, feature_spaces, labels = get_vertical_train_data(
            args.dataset, args.num_clients, return_labels=True
        )
        ...
        train_data, feature_spaces, client_idx, labels = get_hybrid_train_data(
            args.dataset,
            args.num_clients,
            args.overlap_frac_hybrid,
            sample_frac,
            return_labels=True,
        )
        final_train_data = []
...
def main_rf(args):
    ...
    if args.setting == "horizontal":
        ...
        feature_spaces = [
            list(range(train_data[0].shape[1] - 1)) for _ in range(args.num_clients)
        ]
...
def main_spflow(args):
    ...
    if args.setting == "horizontal":
        ...
        train_data, feature_spaces = get_vertical_train_data(
            args.dataset, args.num_clients
        )
        nodes = server.train(train_data, feature_spaces, args)
    elif args.setting == "hybrid":
        sample_frac = None if args.sample_frac == -1 else args.sample_frac
        train_data, feature_spaces, client_idx = get_hybrid_train_data(
            args.dataset, args.num_clients, args.overlap_frac_hybrid, sample_frac
        )
        nodes = server.train(train_data, feature_spaces, args)
...
def main_einsum(args):
    ...
    if args.setting == "horizontal":
        ...
        sample_frac = None if args.sample_frac == -1 else args.sample_frac
        train_data, feature_spaces = get_hybrid_train_data(
            args.dataset,
            args.num_clients,
            args.min_dim_frac,
            args.max_dim_frac,
...
def main(args):
    ...
    if args.cluster_by_label == 1:
        assert (
            args.setting == "horizontal"
        ), "Cannot cluster by label in vertical and hybrid setting"
...
parser.add_argument("--sample-frac", default=-1.0, type=float)
