...
class Avazu(TabularDataset):
    ...
    def _preprocess_train(self):
        ...
        self.train_data.rename(
            columns={"click": "y", "hour_time": "hour"}, inplace=True, errors="raise"
        )
...
class SyntheticDataset(TabularDataset):
    def __init__(self, path) -> None:
        ...
        self.name = "synthetic"
        # test.csv
        self.train_data = pd.read_csv(os.path.join(path, "beta_data_points_500.csv"))
        self.test_data = pd.read_csv(os.path.join(path, "test.csv"))
        (
            self.X_train,
            self.X_valid,
...
class SantanderDataset(TabularDataset):
    ...
    def _preprocess(self):
        ...
        return (
            ...
            y_valid,
            y_valid,
        )  # NOTE: targets only returned for compatibility reasons, not used
...
class DatasetFactory:
    ...
    def load_dataset(self, ds, **ds_kwargs) -> Dataset:
        ...
        if ds == "income":
            ...
            dataset = SyntheticDataset("datasets/synthetic/")
