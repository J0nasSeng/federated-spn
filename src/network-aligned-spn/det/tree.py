...
class DensityTree:
    ...
    def predict(self, x):
        ...
        while not (
            isinstance(curr_node, NonParamLeafNode)
            or isinstance(curr_node, PCLeafNode)
            or isinstance(curr_node, LeafNode)
            or isinstance(curr_node, HistLeafNode)
        ):
    ...
    def cost(self, data):
        ...
        maxs = (
            ...
            + 1e-6
        )  # ensure non-zero difference
        num_cats = np.array(
            [
                len(np.unique(data[:, i]))
                for i in range(len(self.feature_types))
                if self.feature_types[i] == "cat"
            ]
        )
        ord_mins = np.array(
            [
    ...
    def find_cut(self, data):
        ...
        for j in range(data.shape[1]):
            ...
            for u in ufeature:
                ...
                if (
                    left.shape[0] < self.min_leaf_instances
                    or right.shape[0] < self.min_leaf_instances
                ):
                    continue
