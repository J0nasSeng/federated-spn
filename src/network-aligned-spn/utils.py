...
def region_graph_to_spn(
    G: nx.DiGraph, curr_layer, scope_dist_mapping, spn_root=None, num_sums=5
):
    next_layer = []
    spn = spn_root
    if len(curr_layer) == 0:
        ...
        for p in curr_parents:
            if type(p) == Region:
                ...
                if len(p.scope) == 2:
                    ...
                    choice_left = list(
                        np.random.choice(p.scope, int(len(p.scope) / 2), False)
                    )
    ...
    for l in range(1, num_clients):
        for comb in product(*[clusters] * num_clients):
            ...
            if l > 1:
                ...
                relevant_spns = [
                    prods[tuple(prefix)],
                    client_cluster_spns[l][next_node],
                ]
            else:
                # first product layer -> connect all client SPNs of a certain prod_id
                relevant_spns = [
                    client_cluster_spns[i][j] for i, j in enumerate(prod_id)
                ]
