...
class EinetNode:
    def __init__(
        self, dataset, num_epochs=20, num_classes=2, train_algo="em", device=-1
    ) -> None:
        ...
        for subspace, train_loader in self.subspaces:
            ...
            for epoch_count in range(self.num_epochs):
                ...
                for i, (x, y) in enumerate(train_loader):
                    ...
                    if i % 50 == 0:
                        print(
                            f"Epoch {epoch_count+1}/{self.num_epochs}: Batch: {i}/{len(train_loader)} \t {total_ll / len(train_loader)}"
                        )
                self.losses.append(total_ll / len(train_loader))
                print(
                    f"Epoch {epoch_count+1}/{self.num_epochs}: \t {total_ll / len(train_loader)}"
                )
...
class DensityTreeNode:
    ...
    def train_single(self, subspace, fts):
        model = DensityTree(
            8, fts, min_leaf_instances=10, leaf_type="hist", num_bins=2, scope=subspace
        )
...
class FlowNode:
    def __init__(
        self, dataset, spn_structure, num_clusters, setting, glueing, cluster_by_label
    ) -> None:
        self.dataset = dataset
    ...
    def _build_rat_spn(self, subspace, train_data):
        region_graph = utils.random_region_graph(
            0, list(range(train_data.shape[1])), []
        )
        node_types = utils.infer_node_type(train_data, 60)
        dists = {i: nt for i, nt in enumerate(node_types)}
        curr_layer = [
            n for n in region_graph.nodes if len(list(region_graph.pred[n])) == 0
        ]
