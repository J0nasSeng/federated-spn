...
def main(dataset, num_clients, client_id, chk_dir, device):
    ...
    data_loader = get_dataset_loader(
        dataset, num_clients, config.dataset_inds_file, config.data_skew
    )
    ...
    class SPNClient(fl.client.NumPyClient):
        ...
        def fit(self, parameters, cfg):
            ...
            einets, weights = train_mixture(
                rtpt, loaders, config.num_epochs, device, chk_dir
            )
        ...
        def evaluate(self, parameters, cfg):
            ...
            save_image_stack(
                ...
                5,
                5,
                os.path.join("../samples/demo_mnist/", "samples.png"),
                margin_gray_val=0.0,
            )
    ...
    fl.client.start_numpy_client(
        server_address="localhost:{}".format(config.port),
        client=SPNClient(),
...
def init_spn(device):
    ...
    if config.structure == "poon-domingos":
        ...
        graph = Graph.poon_domingos_structure(
            shape=(config.height, config.width), delta=[8], axes=[1]
        )
...
def cluster_data(train_loader, client_id):
    train_data = torch.concat(
        [x.permute((0, 2, 3, 1)) for x, _ in train_loader]
    ).numpy()
    if os.path.isfile(f"./precomputed/clusters/cluster_{client_id}"):
        means, idx, ds_idx = pickle.load(
            open(f"./precomputed/clusters/cluster_{client_id}", "rb")
        )
        logging.info(
            f"Using precomputed clusters at ./precomputed/clusters/cluster_{client_id}"
        )
    ...
    kmeans = MiniBatchKMeans(
        ...
        verbose=3,
        batch_size=1204,
    ).fit(train_data.reshape(train_data.shape[0], -1))
    ...
    pickle.dump(
        (means, idx), open(f"./precomputed/clusters/clusters_{client_id}", "wb")
    )
...
def group_data_by_clusters(train_loader: DataLoader, clusters):
    ...
    for c in np.unique(clusters):
        ...
        loader = DataLoader(
            subset, train_loader.batch_size, num_workers=0, persistent_workers=False
        )
...
def group_data_by_labels(train_loader: DataLoader, num_classes):
    ...
    for _, idx in group_dict.items():
        ...
        loader = DataLoader(
            subs, train_loader.batch_size, num_workers=0, persistent_workers=False
        )
...
def train(
    einet, train_loader, num_epochs, device, chk_path, mean=None, save_model=True
):
    ...
    for epoch_count in range(num_epochs):
        ...
        if save_model and (
            epoch_count > 0 and epoch_count % config.checkpoint_freq == 0
        ):
    ...
    for i, loader in enumerate(train_loaders):  # TODO: check clusters
        ...
        with torch.no_grad():
            ...
            params[..., 3:] = torch.clamp(
                params[..., 3:],
                config.exponential_family_args["min_var"],
                config.exponential_family_args["max_var"],
            )
            params[..., 0:3] += (
                mean.reshape((config.width * config.height, 1, 1, config.num_dims))
                / 255.0
            )
...
def spn_to_param_list(einet: EinsumNetwork.EinsumNetwork):
    ...
    for node in einet.graph.nodes:
        ...
        if type(node) == Graph.Product:
            ...
            type(node) == Graph.DistributionVector
            and len(list(einet.graph.successors(node))) == 0
        ):
    ...
    parameters = [
        p.cpu().detach().numpy() for p in einet.parameters() if p.requires_grad
    ]
