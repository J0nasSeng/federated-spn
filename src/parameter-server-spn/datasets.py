...
def maybe_download_svhn():
    ...
    for file in svhn_files:
        maybe_download(
            "../../../datasets/svhn/", "http://ufldl.stanford.edu/housenumbers/", file
        )
...
class Loader:
    ...
    def partition(self, reuse_json=True):
        ...
        (
            ...
            self.train_data, self.val_data, self.n_clients, skew=self.skew
        )
        train_dict, val_dict = {}, {}
        ...
        json_dict = {"train": train_dict, "val": val_dict, "test": test_inds.tolist()}
...
class ImageNetLoader(Loader):
    def __init__(self, n_clients, indspath, skew=0) -> None:
        ...
        self.train_data = torchvision.datasets.ImageNet(
            "../../../../datasets/imagenet/", split="train", transform=transform
        )
        self.val_data = torchvision.datasets.ImageNet(
            "../../../../datasets/imagenet/", split="val", transform=transform
        )
...
class SVHNLoader(Loader):
    def __init__(self, n_clients, indspath, skew=0) -> None:
        ...
        self.val_data = torchvision.datasets.SVHN(
            "../../../../datasets/svhn/",
            split="test",
            transform=transform,
            download=True,
        )
        self.ex_data = torchvision.datasets.SVHN(
            "../../../../datasets/svhn/",
            split="extra",
            transform=transform,
            download=True,
...
class CelebaLoader(Loader):
    def __init__(self, n_clients, indspath, skew=0) -> None:
        ...
        self.train_data = torchvision.datasets.CelebA(
            "../../../../datasets/celeba/", split="train", transform=transform
        )
        self.val_data = torchvision.datasets.CelebA(
            "../../../../datasets/celeba/", split="valid", transform=transform
        )
...
def partition_data(train_set, val_set, n_clients):
    ...
    train_partitions = random_split(
        train_set, train_lengths, generator=torch.Generator().manual_seed(42)
    )
    val_partitions = random_split(
        validation, val_lengths, generator=torch.Generator().manual_seed(42)
    )
...
def label_distribution_skew(y, partitions, skew=1):
    ...
    for label_idx in range(0, N_labels, n_labels):
        ...
        mask = (
            torch.isin(y, torch.Tensor(np.arange(label_idx, label_idx + n_labels)))
            .cpu()
            .detach()
            .numpy()
        )
...
def partition_skewed(train_set, val_set, partitions, randomise=True, skew=0):
    ...
    if skew == 0:
        ...
        train_selected, train_inds_remain = label_distribution_skew(
            train_set.targets, partitions, skew
        )
        val_selected, val_inds_remain = label_distribution_skew(
            val_set.targets, partitions, skew
        )
